...
def encode(claims, key, algorithm=ALGORITHMS.HS256, headers=None, access_token=None):
    ...
    return sign(claims, key, headers=headers, algorithm=algorithm)
...
def decode(
    ...
    try:
        payload = verify(token, key, algorithms, verify=verify_signature)
...
def get_unverified_header(token):
    ...
    """
    try:
        headers = get_unverified_header_jws(token)
    except Exception:
...
def get_unverified_headers(token):
    ...
    This is simply a wrapper of get_unverified_header_jws() for backwards
    compatibility.
    ...
    Raises:
        JWTError: If there is an exception decoding the token.
    """
    return get_unverified_header_jws(token)
