...
def _unwrap(variable_parts: VariablePartsType):
    ...
    while curr_parts:
        ...
        if var_type == Router._VAR_ANY_NODE:
            var_any.append(part)
            continue

        if var_type == Router._VAR_ANY_BREAK:
            if var_any:
...
_Route = collections.namedtuple("_Route", ["route", "key_parts", "anything"])
RouteResolved = collections.namedtuple("RouteResolved", ["params", "anything", "route"])
...
class Router:
    ...
    def _match(self, parts: Sequence[str]) -> ResolvedRoute:
        ...
        return RouteResolved(
            ...
            params=make_params(
                key_parts=route_match.key_parts, variable_parts=route_variable_parts
            ),
